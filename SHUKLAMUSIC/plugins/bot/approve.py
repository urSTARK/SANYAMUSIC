...
def small(text: str) -> str:
    small_caps_map = str.maketrans(
        ...
        "ᴬᴮᶜᴰᴱᶠᴳᴴᴵᴶᴷᴸᴹᴺᴼᴾQᴿˢᵀᵁⱽᵂˣʸᶻᵃᵇᶜᵈᵉᶠᵍʰᶦʲᵏˡᵐⁿᵒᵖ𐑎ʳˢᵗᵘᵛʷˣʸᶻ",
    )
...
async def send_welcome_message(chat, user):
    ...
    try:
        ...
        await app.send_message(chat.id, f"<blockquote><b> {small('Request Approved')}</b>\n👤 <a href='tg://user?id={user.id}'>{user.first_name}</a></blockquote>")
    finally:
...
async def join_request_handler(_, req: ChatJoinRequest):
    ...
    auto_doc = await AUTOAPPROVE_DB.find_one({"chat_id": chat.id}) or {"enabled": True}
    auto_enabled = auto_doc.get("enabled", True)
    ...
    try:
        await app.send_message(
            chat.id,
            f"<blockquote><b>📥 {small('New Join Request')}</b>\n\n"
            f"👤 ᴜꜱᴇʀ: <b>{user.first_name}</b>\n"
            f"🆔 ɪᴅ: <code>{user.id}</code></blockquote>",
            reply_markup=InlineKeyboardMarkup(
                [[
                    InlineKeyboardButton(" ᴀᴄᴄᴇᴘᴛ", callback_data=f"acc_{user.id}_{chat.id}"),
                    InlineKeyboardButton(" ᴅɪꜱᴍɪꜱꜱ", callback_data=f"dis_{user.id}_{chat.id}"),
                ]]
            ),
        )
    except:
...
async def approvestatus_cmd(_, m: Message):
    ...
    auto_doc = await AUTOAPPROVE_DB.find_one({"chat_id": m.chat.id}) or {"enabled": True}
    ...
    auto_status = "🟢 ON" if auto_doc.get("enabled", True) else "🔴 OFF"
    ...
    await m.reply_text(
        ...
        f"<b>⚙️ Auto Approve:</b> {auto_status}\n"
...
async def global_approvestatus_cmd(_, m: Message):
    ...
    if not all_chat_ids:
        return await m.reply_text("<blockquote><b>ℹ️ No chats found in DB.</b></blockquote>")
    ...
    for cid in sorted(all_chat_ids):
        try:
            ...
            title = str(cid)
        a_status = "🟢 ON" if auto_map.get(cid, True) else "🔴 OFF"
